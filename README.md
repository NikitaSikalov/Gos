...
#### Примечения к пониманию условию задачи
...
Идея решения строится на том, что весь наш лог-файл токенизируется на отдельные токены, а из них строится далее граф. 
...
Эффективность хранения в памяти достигается за счет того, что повторяющиеся токены будут храниться будут храниться в ограниченном количестве. 
Помимо этого в нашем графе присутствует так назавыемая `START` стартовая вершина и `END` концевая вершина, которые не соответсвуют
ни одному токену. Каждая строчка лога в нашем графе начинается из стартовой вершины, а заканчивается в концевой
вершине нашего лог графа, а сама строчка лога по сути является путем в таком графе из стартовой вершины в концевую.

Заметим, что в лог файле важен порядок строк, поэтому в каждой вершине поддерживается очередь соседних вершин.
Таким образом, когда мы выбираем какую вершину обходить следующией, мы просто достаем из очереди соседних вершин очередную вершину.
Дополнительно использовалась оптимизация, если в очереди соседних вершин находится несколько подряд идущих одних тех же вершин.
...
По сути мы делаем топологическую сортировку вершин нашего графа, а потом в обратном порядке топологической сортировки записываем
вершины в наш протобуф (то есть концевая вершина `END` будет записана первой, а стартовая `START` последней). Чтобы индентифицировать
вершины в сериализованном состоянии мы добавляем вершинам уникальные id с помощью `reiterpret_cast` указателя вершины к uint64 
